...
namespace SIPDataCollector.Utilites
{
    class ConfigurationData 
    {
        ...
        public static void LoadConfig()
        {
            ...
            try
            {
                ConntnString = ApplicationSettings.CMDbConn;
                ...
                DashboardRefreshTime = Convert.ToInt32(ApplicationSettings.DashboardRefreshTime);
                acceptableSL = Convert.ToInt32(ApplicationSettings.acceptableSL);
                DBRefreshTime = Convert.ToInt32(ApplicationSettings.DBRefreshTime);
                TmacServers = ApplicationSettings.TmacServers?.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries)?.ToList();
                ReloadConfigTime = Convert.ToInt32(ApplicationSettings.ReloadConfigTime);
                //jsonPath = ConfigurationManager.AppSettings["JsonFilePath"];
                GetAgentSkillInfoConfig = ApplicationSettings.GetAgentSkillInfo;
                isVDN = Convert.ToBoolean(ApplicationSettings.isVDNMonitor);
                ExtraAUXCodes = ApplicationSettings.ExtraAUXCodes?.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries)?.ToList();
        ...
        public static void Channel()
        {
            ...
            try
            {
                ...
                var channel = ApplicationSettings.channelName.Split(';');
                List<BCMSInstanceData> bcmsdata = new List<BCMSInstanceData>(); 
                foreach(var item in channel)
                {
                    bcmsdata.Add(new BCMSInstanceData
                    {
                        ChannelName = item.Split(':')[0],
                        SkillId = item.Split(':')[1]
                    }) ;
                }
                //var section = (SIPDataCollector.Utilites.BcmsSIPConfigSection)ConfigurationManager.GetSection("TRealTimeDataServiceSettings");
                foreach (BCMSInstanceData data in bcmsdata)
                {
            ...
            try
            {
                ...
                if (skillList != null && skillList.Count() >= 0 && !string.IsNullOrEmpty(skillList))
                {
                    ...
                    if ((strArrays != null ? true : strArrays.Length != 0))
                    {
                        if (strArrays[0] != "")
                        {
                            List<SMSAPI.HuntGroupType> result =  SMSAPIProxy.GetSkills();
